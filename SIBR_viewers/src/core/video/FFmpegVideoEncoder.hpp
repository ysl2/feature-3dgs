...
//#define HEADLESS
		...
		const AVOutputFormat* fmt = nullptr; ///< Output format.
		AVStream* video_st = nullptr; ///< Output stream.
		AVCodecContext* pCodecCtx = nullptr; ///< Codec context.
		const AVCodec* pCodec = nullptr; ///< Codec.
		AVPacket * pkt = nullptr; ///< Encoding packet.
